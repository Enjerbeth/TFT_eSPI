...
// === AERO ZERO-HEAP VENDORING ===
#define AERO_MAX_FONT_GLYPHS 128
static uint16_t aero_gUnicode[AERO_MAX_FONT_GLYPHS];
static uint8_t  aero_gHeight[AERO_MAX_FONT_GLYPHS];
static uint8_t  aero_gWidth[AERO_MAX_FONT_GLYPHS];
static uint8_t  aero_gxAdvance[AERO_MAX_FONT_GLYPHS];
static int16_t  aero_gdY[AERO_MAX_FONT_GLYPHS];
static int8_t   aero_gdX[AERO_MAX_FONT_GLYPHS];
static uint32_t aero_gBitmap[AERO_MAX_FONT_GLYPHS];
// ================================
...
void TFT_eSPI::loadMetrics(void)
{
  ...
  if (gFont.gCount > AERO_MAX_FONT_GLYPHS) {
    // Avoid crashing, but the text won't render if it exceeds the static pool
    gFont.gCount = AERO_MAX_FONT_GLYPHS;
  }

  gUnicode  = aero_gUnicode;
  gHeight   = aero_gHeight;
  gWidth    = aero_gWidth;
  gxAdvance = aero_gxAdvance;
  gdY       = aero_gdY;
  gdX       = aero_gdX;
  gBitmap   = aero_gBitmap;
